...
var clock = new THREE.Clock; // clock varables //
...
glowringani(), scanlineani(), glowstatic(), basiccrossani();
//animation cleaning ((call these as an else statement to clean an animation))
scanlineclean(), glowstaticclean(), glowringaniclean(), basiccrossclean();
...
function animate() {
  ...
  //=================================================
    ...
    if(camera.position.x > 65) { // camera going right
      ...
      sideLXlineL = (-corrector / (Math.PI * 1.94)) - 21; // continuation of (TBL)
  ...
  //glowstatic();
  if(tock == 1){ var testvalue = 28}
  basiccrossani();
  timerloop1 = 0.25 // speed of the animation
    ...
    if (i <= 128 + delaytimer + 24 + finaldelaytime){ i = i + timerloop1 , ic = i - ( delaytimer ) // change this for different rendering techniques
      ...
      if (i <= 128 + delaytimer){ scene.remove(lineMmesh[ic]); // 28 - 42 < widened lines
        if (i <= 128){
          ...
          if (i >= 28 && i <= 42){ lineMgeometry[i] = new THREE.Geometry(), lineMgeometry[i].vertices.push(
